...
class Calibrator(Model):
    ...
    def fit(self, series, caller, error_metric, *args, **kwargs):
        """Fit the calibrator on the given series"""
        raise NotImplementedError()

    def probabilize(self, prediction, series, *args, **kwargs):
        """Make a prediction originating from a given series probabilistic, according to the calibrator.
        The arguments are the same provided to the calibrated model's predict() call"""

        # Note: parameters 
        raise NotImplementedError()

    def predict(self, series, *args, **kwargs):
        """Disabled. Calibrators detectors can be used only with the ``probabilize()`` method."""
...
class ErrorDistributionCalibrator(Calibrator):
    """A model calibrator based on the (global) error distribution."""
...
class CHEMCalibrator(Calibrator):
    """A model calibrator based on Calibrated Heteroscedastic Error Modeling (CHEM).
