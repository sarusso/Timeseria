...
def _compute_new(type,
    ...
    if type=='point':
        ...
        if data_loss == 1: 
            ...
            if len(series) != 2:
                raise ConsistencyException('Got full data loss with series length != 2 for prev & next (got "{}"), this should never happen.'.format(len(series)))
...
class Resampler(Transformation):
    ...
    def process(self, data_time_point_series, from_dt=None, to_dt=None, validity=None,
        ...
        # Set "from"
        from_t = data_time_point_series[0].t  - (self.time_unit.duration_s() /2)
        ...
        # Set "to"
        to_t = data_time_point_series[-1].t  + (self.time_unit.duration_s() /2)
        ...
        for data_time_point in data_time_point_series:
            ...
            if data_time_point.t >= slot_end_t:
                ...
                if data_time_point.dt >= to_dt:
                    ...
                    if data_time_point.dt == to_dt and include_extremes:
                        ...
                        # .. and append results
                        if dataTimePoint:
                            # if first and dataTimePoint.data_loss != 0 and not include_extremes:
                            #    pass
                            resampled_data_time_point_series.append(dataTimePoint)
